#include "../napi.h"
#include "ecdh_gm.hpp"
#include "ecdh_gm_key_agreement_napi.hpp"
...
napi_value key_agreement(napi_env env, napi_callback_info args) {
  ...
  if (value_type != napi_function) {
    napi_throw_type_error(env, "ERR_INVALID_ARG_TYPE",
                          "The fourth argument must be a function");
    return nullptr;
  }
  ...
  NAPI_CALL(env, napi_get_value_string_utf8(env, argv[2], curve_name,
                                            curve_namel, &datal));

  napi_ref callback;
  NAPI_CALL(env, napi_create_reference(env, argv[3], 1, &callback));

  napi_value resource_name;
  NAPI_CALL(env, napi_create_string_utf8(env, "keyAgreement", NAPI_AUTO_LENGTH,
                                         &resource_name));

  key_agreement_data* worker_data = new key_agreement_data;
  ...
  NAPI_CALL(env, napi_create_async_work(
                     env, nullptr, resource_name, key_agreement_execute,
                     key_agreement_complete, worker_data, &worker_data->work));
...
void key_agreement_execute(napi_env env, void* data) {
  key_agreement_data* worker_data = static_cast<key_agreement_data*>(data);
  ...
  if (ret == 1 && status == 0) {
    len = EC_POINT_point2buf(group, shared_secret,
                             POINT_CONVERSION_UNCOMPRESSED, &buffer, nullptr);
    if (len == 0) {
      status = 5;
    }
  }
...
void key_agreement_complete(napi_env env, napi_status status, void* data) {
  ...
  napi_value shared_secret;
  NAPI_CALL_RETURN_VOID(
      env, napi_create_buffer_copy(env, worker_data->shared_secret.size(),
                                   worker_data->shared_secret.data(), nullptr,
                                   &shared_secret));
